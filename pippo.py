    ...
    def on_gazedata(self, error, gaze):
        ...
        if self.ETCoord.state_variable == 11:
            if self.ETCoord.gaze_POR[0] >= self.ETCoord.back_hall_butt_pos[0] and self.ETCoord.gaze_POR[1] >= \
                    self.ETCoord.back_hall_butt_pos[2] and self.ETCoord.gaze_POR[0] <= self.ETCoord.back_hall_butt_pos[1] and \
            ...
            if self.i_back_home >= self.ETCoord.fixation_time:
                print "Back to hall"
                self.ETCoord.old_state_variable=self.ETCoord.state_variable
        ...
        if self.ETCoord.state_variable == 0 or self.ETCoord.state_variable == 1:
            ...
            if self.i_save >= self.ETCoord.fixation_time:
                print "Save position"
            #stato 2 per save e recall position
                self.ETCoord.old_state_variable=self.ETCoord.state_variable
...
class Control:
    ...
    def draw(self,ctx):
        ...
        #self.ret_val=True #, self.ETCoord.img = self.cam.read()
        ...
        if self.ETCoord.state_variable==0 or self.ETCoord.state_variable==2 and self.ETCoord.old_state_variable==0 :
            self.pixbuf_bk = gtk.gdk.pixbuf_new_from_file('UI-Official_trasp DEFINITIVA.png')
            ...
            self.pixbuf_bk = gtk.gdk.pixbuf_new_from_file('Hall.png')
            ctx.set_source_pixbuf(self.pixbuf_bk, 0, 0)
            ctx.paint()
            ctx.restore()
            ...
            ctx.restore()
            ctx.save()
            ctx.set_source_pixbuf(gtk.gdk.pixbuf_new_from_file('red_ball_2.0.png'), self.ETCoord.ball_xpos[self.ETCoord.i_r], 805)
            ctx.paint()
            ctx.restore()
            ctx.save()
            ctx.set_source_pixbuf(gtk.gdk.pixbuf_new_from_file('blue_ball_2.0.png'), self.ETCoord.ball_xpos[self.ETCoord.i_b], 419)
            ctx.paint()
            ctx.restore()
            ctx.save()
            ctx.set_source_pixbuf(gtk.gdk.pixbuf_new_from_file('orange_ball_2.0.png'), self.ETCoord.ball_xpos[self.ETCoord.i_o], 612.5)
            ctx.paint()
            ctx.restore()
            ctx.save()
            ctx.set_source_pixbuf(gtk.gdk.pixbuf_new_from_file('arrow.png'), 38, self.ETCoord.arrow_ypos[self.ETCoord.i_a])
            ctx.paint()
            ctx.restore()
            ctx.save()
        ...
        if self.ETCoord.state_variable == 1:
