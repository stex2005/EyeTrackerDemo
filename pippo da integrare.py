...
sys.path.append("C:\Users\\andre\Desktop\BRIDGE\BRIDGE_ET\\tobii-analytics-sdk-3.1.0-win-Win32\Python27\Modules")
...
from BridgeConf import BridgeCoordClass
## aggiunto MOD perchè in BridgeETConfMOD sono state aggiunte le posizioni della nuova interfaccia
...
global save2file_POR
global save2file_p0
global save2file_DemoCoord
save2file_POR       = []
save2file_p0        = []
save2file_DemoCoord = []
...
class Thread_TrackingClass(threading.Thread):
    ...
    def on_gazedata(self, error, gaze):
        ...
        self.ETCoord.gaze_POR = ((gaze.LeftGazePoint2D.x+gaze.RightGazePoint2D.x)/2, (gaze.LeftGazePoint2D.y+gaze.RightGazePoint2D.y)/2)

        save2file_POR.append(self.ETCoord.gaze_POR[0])
        save2file_POR.append(self.ETCoord.gaze_POR[1])
        save2file_p0.append(self.Coord.p0[0])
        save2file_p0.append(self.Coord.p0[1])
        save2file_p0.append(self.Coord.p0[2])
        save2file_DemoCoord.append(self.ETCoord.demo_coord[0])
        save2file_DemoCoord.append(self.ETCoord.demo_coord[1])
        save2file_DemoCoord.append(self.ETCoord.demo_coord[2])


                '''START PRE-CONTROL'''
         if self.ETCoord.state_variable == 10:
            if self.ETCoord.gaze_POR[0] >= self.ETCoord.precontrol_butt_pos[0] and self.ETCoord.gaze_POR[1] >= self.ETCoord.precontrol_butt_pos[2] and self.ETCoord.gaze_POR[0] <= self.ETCoord.precontrol_butt_pos[1] and \
                    self.ETCoord.gaze_POR[1] <= self.ETCoord.precontrol_butt_pos[3]:
                self.i_control = self.i_control + 1
            else:
            ...
            if self.i_precontrol >= self.ETCoord.fixation_time:
                print "Start Pre-Control"
                ...
                '''SETTINGS'''
        if self.ETCoord.state_variable == 10:
            if self.ETCoord.gaze_POR[0] >= self.ETCoord.settings_butt_pos[0] and self.ETCoord.gaze_POR[1] >= self.ETCoord.settings_butt_pos[2] and self.ETCoord.gaze_POR[0] <= self.ETCoord.settings_butt_pos[1] and \
            ...
            if self.i_settings >= self.ETCoord.fixation_time:
                print "Start Pre-Control"
                ...
                '''KILL THE PROGRAM'''
        if self.ETCoord.state_variable == 10:
            if self.ETCoord.gaze_POR[0] >= self.ETCoord.off_tracker_butt_pos[0] and self.ETCoord.gaze_POR[1] >= self.ETCoord.off_tracker_butt_pos[2] and self.ETCoord.gaze_POR[0] <= self.ETCoord.off_tracker_butt_pos[1] and \
            ...
            if self.i_off_tracker >= self.ETCoord.fixation_time:
                print "PROGRAM KILLED"
                self.ETCoord.state_variable = -1 #Kill the program
                self.i_off_tracker = 0
                self.i_off_tracker_reset = 0
        ...
        if self.ETCoord.state_variable == 11:
            ...
            if self.ETCoord.state_variable == 11
                for i in range(0,3):
                    ...
                    if  self.i_settings_joystick[i] >= self.ETCoord.fixation_time:
        ...
        ''' EMERGENCY EXIT (x icon) '''
        if self.ETCoord.gaze_POR[0]>=self.ETCoord.emergency_butt_pos[0] and self.ETCoord.gaze_POR[1]>=self.ETCoord.emergency_butt_pos[2] and self.ETCoord.gaze_POR[0]<self.ETCoord.emergency_butt_pos[1] and self.ETCoord.gaze_POR[1]<   self.ETCoord.emergency_butt_pos[3]:
        ...
        if self.i_exit >= self.ETCoord.fixation_time:
        ...
        if self.ETCoord.gaze_POR[0] >= self.ETCoord.save_butt_pos[0] and self.ETCoord.gaze_POR[1] >= self.ETCoord.save_butt_pos[2] and self.ETCoord.gaze_POR[0] <= self.ETCoord.save_butt_pos[1] and \
        ...
        if self.i_save >= self.ETCoord.fixation_time:
            print "Save position"
            self.i_save = 0
            self.i_save_reset = 0
        ...
        '''RECALL POSITION'''
        ##DEFINIRE POSIZIONI
        if self.ETCoord.gaze_POR[0] >= self.ETCoord.recall_butt_pos[0] and self.ETCoord.gaze_POR[1] >= self.ETCoord.recall_butt_pos[2] and self.ETCoord.gaze_POR[0] <= self.ETCoord.recall_butt_pos[1] and \
        ...
        if self.i_recall >= self.ETCoord.fixation_time:
            print "Recall position"
            self.i_recall = 0
            self.i_recall_reset = 0
        ...
        if self.ETCoord.state_variable == 0:
            if self.ETCoord.gaze_POR[0]>=self.ETCoord.standby_butt_pos[0] and self.ETCoord.gaze_POR[1]>=self.ETCoord.standby_butt_pos[2] and self.ETCoord.gaze_POR[0]<self.ETCoord.standby_butt_pos[1] and self.ETCoord.gaze_POR[1]<self.ETCoord.standby_butt_pos[3]:
        ...
        if self.ETCoord.state_variable == 1 and self.delay_start >= 180:
            ...
            if self.ETCoord.gaze_POR[0] >= self.ETCoord.webcam_pos[0]/self.ETCoord.screen_w and \
             self.ETCoord.gaze_POR[0] <= self.ETCoord.webcam_pos[0]/self.ETCoord.screen_w + self.ETCoord.webcam_size_norm_w and \
             self.ETCoord.gaze_POR[1] >= self.ETCoord.webcam_pos[1]/self.ETCoord.screen_h and \
             self.ETCoord.gaze_POR[1] <= self.ETCoord.webcam_pos[1]/self.ETCoord.screen_h + self.ETCoord.webcam_size_norm_h:
                ...
                if self.check_quadrato and self.counter < self.ETCoord.fixation_time:
    			    ...
    			    if self.ETCoord.gaze_POR[0] > self.camp_primo[0]-self.ETCoord.fixation_area and self.ETCoord.gaze_POR[0]<self.camp_primo[0]+self.ETCoord.fixation_area and self.ETCoord.gaze_POR[1] > self.camp_primo[1]-self.ETCoord.fixation_area and self.ETCoord.gaze_POR[1]<self.camp_primo[1]+self.ETCoord.fixation_area:
        ...
        if self.ETCoord.state_variable == 2:
            ...
            if self.ETCoord.gaze_POR[0] >= self.ETCoord.webcam_pos[0]/self.ETCoord.screen_w and \
             self.ETCoord.gaze_POR[0] <= self.ETCoord.webcam_pos[0]/self.ETCoord.screen_w + self.ETCoord.conf_box_w and \
            ...
            if  self.i_ok >= self.ETCoord.fixation_time:
                ...
                self.i_ok_reset = 0
                self.i_no_reset = 0
                print 'Fixation bar: ', self.ETCoord.screen_w*self.ETCoord.bar[0]-self.ETCoord.webcam_pos[0], self.ETCoord.screen_h*self.ETCoord.bar[1]-self.ETCoord.webcam_pos[1]
                print 'Target: ', self.ETCoord.fixation_target[0]+25,'    ',self.ETCoord.fixation_target[1]+25
        ...
        if self.ETCoord.state_variable == 4:
            ...
            if self.ETCoord.gaze_POR[0]>=0.8 and self.ETCoord.gaze_POR[1]>=0 and self.ETCoord.gaze_POR[0]<1 and self.ETCoord.gaze_POR[1]<0.2:
                self.i_ok = self.i_ok+1
            else:
                if self.i_ok_reset >= self.ETCoord.button_fixation_tol:
                    self.i_ok = 0
...
class Calibration:
    def __init__(self):
        ...
        self.points = [(0.1,0.1), (0.5,0.1), (0.9,0.1) , (0.1,0.5), (0.5,0.5), (0.9,0.5), (0.1,0.9), (0.5,0.9), (0.9,0.9)]
        self.point_index = -1
...
class Control:
    ...
    def __init__(self, ETCoord, Coord):
        ...
        self.pixbuf_bk = gtk.gdk.pixbuf_new_from_file('UI_Official.png')
        # load confirmation box image
        self.pixbuf_conf_box = gtk.gdk.pixbuf_new_from_file('conf_box.png')
    ...
    def draw(self,ctx):
        ...
        if self.ret_val == True:
            ...
            self.ETCoord.img = cv2.resize(self.ETCoord.img, (self.ETCoord.webcam_size_w, self.ETCoord.webcam_size_h), interpolation = cv2.INTER_AREA)
            self.pixbuf = None
            ...
            if self.ETCoord.state_variable == 2:
                ctx.save()
                ctx.set_source_pixbuf(self.pixbuf_conf_box, self.ETCoord.webcam_pos[0], 0)
                ctx.paint()
                ctx.restore()

        if self.ETCoord.state_variable == 1 or self.ETCoord.state_variable == 2:
            ctx.set_source_rgb(0.2,0.2,0.2)
            ctx.rectangle(self.ETCoord.webcam_pos[0]+self.ETCoord.fixation_target[0], self.ETCoord.webcam_pos[1]+self.ETCoord.fixation_target[1], 50,50)
            ctx.fill()



        # se sto usando il joystick e sono in modalita DEMO mostro exo feedback
        if self.ETCoord.demo == True:
            # print 'prova: ', self.ETCoord.state_variable

            if self.ETCoord.state_variable == 3 or self.ETCoord.state_variable == 4:
        		# rettangolo base (bianco)
        		ctx.set_source_rgb(255,255,255)
        		ctx.rectangle(self.ETCoord.webcam_pos[0], self.ETCoord.webcam_pos[1], self.ETCoord.webcam_size, self.ETCoord.webcam_size)
        		ctx.fill()

        		# spalla
        		ctx.set_source_rgb(0,0,0)
        		# x
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+170, self.ETCoord.webcam_pos[1]+170, 20,30)
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+780-170, self.ETCoord.webcam_pos[1]+170, 20,30)
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+170, self.ETCoord.webcam_pos[1]+170+10, 440, 10)
        		# y
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+170, self.ETCoord.webcam_pos[1]+2*170, 20,30)
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+780-170, self.ETCoord.webcam_pos[1]+2*170, 20,30)
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+170, self.ETCoord.webcam_pos[1]+2*170+10, 440, 10)
        		# z
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+170, self.ETCoord.webcam_pos[1]+3*170, 20,30)
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+780-170, self.ETCoord.webcam_pos[1]+3*170, 20,30)
        		ctx.rectangle(self.ETCoord.webcam_pos[0]+170, self.ETCoord.webcam_pos[1]+3*170+10, 440, 10)
        		ctx.fill()
        		...
        		# x
        		ctx.arc (self.ETCoord.webcam_pos[0]+400+self.ETCoord.demo_coord[0], self.ETCoord.webcam_pos[1]+170+10, 10, 0, math.pi*2)
        		ctx.fill()
        		# y
        		ctx.arc (self.ETCoord.webcam_pos[0]+400+self.ETCoord.demo_coord[1], self.ETCoord.webcam_pos[1]+2*170+10, 10, 0, math.pi*2)
        		ctx.fill()
        		# z
        		ctx.arc (self.ETCoord.webcam_pos[0]+400+self.ETCoord.demo_coord[2], self.ETCoord.webcam_pos[1]+3*170+10, 10, 0, math.pi*2)
        ...
        if self.ETCoord.gaze_POR[0] >= 0 and self.ETCoord.gaze_POR[1] >= 0:
           	ctx.rectangle(self.ETCoord.screen_w*self.ETCoord.gaze_POR[0], self.ETCoord.screen_h*self.ETCoord.gaze_POR[1], 20, 20)
           	ctx.fill()

        if self.ETCoord.state_variable == 2: # se ho trovato un punto di fissazione sulla webcam sulla webcam
        	ctx.set_source_rgb(0,255,255)
        	ctx.rectangle(self.ETCoord.screen_w*self.ETCoord.bar[0], self.ETCoord.screen_h*self.ETCoord.bar[1], 30, 30)
        	ctx.fill()
...
class EyetrackerBrowser:
    ...
    def __init__(self, ETCoord, Coord, trackstatus):
        ...
        self.button_sc = gtk.Button("Start control")
    ...
    def on_sc_button_clicked(self, button):
        ...
        if self.eyetracker is not None:
            ...
            # NOTTE self.control.run(self.eyetracker, lambda status, message: glib_idle_add(self.on_calib_done, status, message))
...
if __name__ == "__main__":
    ...
    eb = EyetrackerBrowser(ETCoord, Coord, trackstatus)
    eb.main()
